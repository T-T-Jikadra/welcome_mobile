...
class _ModuleViewPageState extends State<companyView> {
              ...
              child: SlidableAutoCloseBehavior(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    var companyModel = _filteredData[index];
                    formattedData =
                        _formatData(companyModel, widget._DataFormat);
                        ...
                        startActionPane:
                          ...
                          SlidableAction(
                              onPressed: (context) async {
                                await del(companyModel).then((onValue) {
                                  loadData();
                                });
                              },
  ...
  void updateStatus(id, companyModel) {
    Dialogs.bottomMaterialDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              _updateCompany(id, companyModel).then((onValue) {
                loadData();
              });
              Navigator.pop(context);
              //  DeleteData(id);
  ...
  Future<void> _updateCompany(int id, companyModel model) async {
    setState(() {
      companyModel name = _company.firstWhere((item) => item.id == id);
  ...
  Future<void> _saveCompanyToFile() async {
    ...
    List<Map<String, dynamic>> taskMapList =
        _company.map((task) => task.toMap()).toList();
    String jsonString = jsonEncode(taskMapList);

    await file.writeAsString(jsonString);
  }

  Future<bool> del(companyModel) async {
    widget.onDel(companyModel.id);
    return true;
  }
  ...
  Future<bool> loadData() async {
    ...
    if (await file.exists()) {
      ...
      setState(() {
        _company = decoded.map((data) => companyModel.fromMap(data)).toList();
        _companyDetails = List<companyModel>.from(_company);

        _companyDetails.sort((a, b) {
          return a.name.compareTo(b.name);
        });

        _filteredData = _companyDetails;
      });
    } else {
  ...
  Padding buildFilterListView() {
    ...
    if (_companyDetails.isEmpty) {
      ...
      var firstItem = _companyDetails[0];
  ...
  SearchData(s) {
    ...
    setState(() {
      _filteredData = _companyDetails.where((item) {
        return item
                .toMap()[_filterCol]
                ?.toString()
                .toLowerCase()
